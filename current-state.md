...
- **Warm Standby HA Sync Automation Active**:
  ...
  2. **Job #7: Vaultwarden Standby Restore** (`17 * * * *`): Extracts latest backup on Synology, atomic directory swap to `/volume1/docker/vaultwarden-standby/data`, restarts standby container via Portainer API, and verifies health (`/alive` -> 200).
  3. **Job #8: NPM Config Sync to Synology** (`22 * * * *`): Hourly rsync of `/data` and `/etc/letsencrypt` to `/volume1/docker/nginx-proxy-standby/`, reloads nginx on standby via Portainer exec endpoint, and verifies UI responds on port 8081.
...
- **Container**: `benny2168/crondock:1.3.1` running with mounts for `/data`, `/var/run/docker.sock`, `/root/.ssh` (id_ed25519 + known_hosts), `/host-backups`, `/nginx-proxy-src/data`, and `/nginx-proxy-src/letsencrypt`.
- **Validation**: All 7 jobs tested and verified green in production. Standby Vaultwarden and NPM instances confirmed healthy on Synology.
